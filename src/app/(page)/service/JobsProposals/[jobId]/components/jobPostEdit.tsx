...
const JobPostEdit: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (job.budget === null || job.budget === undefined || job.budget < 0)
      newErrors.budget = "Budget must be a non-negative number";
    if (!job.paymentType || job.paymentType.trim() === "")
      newErrors.paymentType = "Payment type is required";
    if (!job.duration || job.duration.trim() === "")
      newErrors.duration = "Duration is required";
    if (!job.experienceLevel || job.experienceLevel.trim() === "")
      newErrors.experienceLevel = "Experience level is required";
    if (!job.deadline || isNaN(new Date(job.deadline).getTime())) {
