...
interface Proposal {
  ...
  description: string;
  status: string;
  image: string; // This will contain the PDF URL
  userId: string;
}
...
const JobProposal: React.FC = () => {
  ...
  const handleProposalAction = async (
    ...
    try {
      await axiosInstance.post(`${PROPOSALSTATUS}/${proposalId}`,action);
  ...
  const renderProposalCard = (proposal: Proposal) => (
    ...
    >
      ...
      <p className="text-sm mb-4 line-clamp-3">{proposal.description}</p>
      <div className="grid grid-cols-2 gap-2 mb-4">
      ...
      <button
        ...
        className="mt-4 bg-cyan-600 text-white px-4 py-2 rounded-full hover:bg-cyan-700 transition-colors duration-300"
      >
        View Details
      </button>
  ...
  return (
    <div className="min-h-screen p-8 font-sans bg-gradient-to-br from-gray-900 via-purple-900 to-black text-white">
      <h1 className="text-4xl mb-8 text-center text-cyan-400">Job Dashboard</h1>

      <div className="flex justify-center mb-8">
        <button
          onClick={() => setActiveSection("posted")}
          className={`px-4 py-2 rounded-l-full ${
            activeSection === "posted" ? "bg-cyan-600" : "bg-gray-700"
          }`}
        >
          Posted Jobs
        </button>
        <button
          ...
          className={`px-4 py-2 ${
            activeSection === "sent" ? "bg-cyan-600" : "bg-gray-700"
          }`}
        >
          Sent Proposals
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-r-full ${
            activeSection === "received" ? "bg-cyan-600" : "bg-gray-700"
          }`}
        >
          Received Proposals
        </button>
      ...
      {selectedProposal && (
        ...
        >
          ...
          >
            <h2 className="text-2xl mb-4 text-cyan-400">Proposal Details</h2>
            ...
            <div className="mb-4">
              <a
                ...
                className="bg-cyan-600 text-white px-4 py-2 rounded-full hover:bg-cyan-700 transition-colors duration-300"
              >
                View Resume PDF
              </a>
            </div>
